...
parser.add_argument('--weights', type=str, default='diehl')
...
elif args.weights == 'diehl':
    w = np.load('./weights/XeAe.npy')
    theta = np.load('./weights/theta_A.npy')    
elif args.weights == 'iedm':
    w = np.load('./trained/XeAe.npy')
    theta = np.load('./trained/theta_A.npy')
else:
    assert(False)
...
while ex < NUM_EX:
    ...
    for s in range(active_steps):
        ...
        if args.train:
            rates = x_train[ex] * 32.0 * input_intensity
            labels[ex] = y_train[ex]
        else:
            rates = x_test[ex] * 32.0 * input_intensity
            labels[ex] = y_test[ex]
